...
# muOS SD Card Enrollment Trigger — detect, parse, and import setup.json
# (setup.json at the SD1 exFAT root, /mnt/mmc — what a PC reader sees).
...
# The boot daemon and a Task Toolkit interactive enrollment can run
# CONCURRENTLY (field-found on NextUI: the daemon saw device_name
# mid-enrollment and raced into cold start against a half-pushed
# remote); on muOS the boot daemon and a Task Toolkit tap can race the
# same way. mkdir is the
...
# entry point (boot daemon vs Task Toolkit tap), then run the real import.
