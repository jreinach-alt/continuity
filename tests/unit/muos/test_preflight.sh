...
# Unit tests for src/platforms/muos/preflight.sh (the doctor)
set -e
...
. "$PROJECT_ROOT/src/platforms/muos/preflight.sh"
...
cp "$PROJECT_ROOT/config/platform_maps/muos.json" "$TEST_TMPDIR/platform_map.json"
...
assert_contains "mapping check passes with real map" "$R10" "watched dirs; Snes9x probe translates"
...
# --- Test 13: muOS version signals — recorded verbatim, trusted never ---
# (Acceptance I9: the reference device's two version files disagree.)
printf 'PRETTY_NAME="muOS 2410 Banana"\n' > "$TEST_TMPDIR/os-release"
printf '2502.0_PIXIE\n' > "$TEST_TMPDIR/version.txt"
R13="$TEST_TMPDIR/r13.txt"
rc=0; PF_OS_RELEASE="$TEST_TMPDIR/os-release" \
     PF_MUOS_VERSION_FILE="$TEST_TMPDIR/version.txt" pf_run "$R13" || rc=$?
assert_contains "both version signals verbatim" "$R13" \
    "os-release='muOS 2410 Banana' version.txt='2502.0_PIXIE'"
assert_contains "version marked diagnostics-only" "$R13" "never branched on"
# absent files degrade, never fail
R13b="$TEST_TMPDIR/r13b.txt"
rc=0; PF_OS_RELEASE="$TEST_TMPDIR/nope" \
     PF_MUOS_VERSION_FILE="$TEST_TMPDIR/nope2" pf_run "$R13b" || rc=$?
assert_contains "absent version files reported absent" "$R13b" "os-release='absent'"

# --- Test 14: path resolution + boot-hook recon are informational ---
assert_contains "path resolution surfaced" "$R13" "saves=$CONTINUITY_SAVES_ROOT"
mkdir -p "$TEST_TMPDIR/initd" "$TEST_TMPDIR/muxscript"
printf '#!/bin/sh\n' > "$TEST_TMPDIR/initd/S01muos"
printf '#!/bin/sh\n' > "$TEST_TMPDIR/muxscript/system.sh"
R14="$TEST_TMPDIR/r14.txt"
rc=0; PF_INITD="$TEST_TMPDIR/initd" \
